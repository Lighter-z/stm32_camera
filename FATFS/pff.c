...
FRESULT pf_write (
	const void* buff,	/* Pointer to the data to be written 写入的数据地址tbuf*/
	WORD btw,			/* Number of bytes to write (0:Finalize the current write operation)要写入的数据个数 */
...
)
{
	...
	*bw = 0;
	...
	if (!btw) {		/* Finalize request */
		if ((fs->flag & FA__WIP) && disk_writep(0, 0)) goto fw_abort;//写结束
		fs->flag &= ~FA__WIP;
		return FR_OK;
	...
	while (btw)	{									/* Repeat until all data transferred */
		if (((WORD)fs->fptr % 512) == 0) {			/* On the sector boundary? */
			...
			fs->dsect = sect + cs;
			if (disk_writep(0, fs->dsect)) goto fw_abort;	/* Initiate a sector write operation 写开始*/
			fs->flag |= FA__WIP;
		}
		wcnt = 512 - ((WORD)fs->fptr % 512);		/* Number of bytes to write to the sector */
		if (wcnt > btw) wcnt = btw;
		if (disk_writep(p, fs->dsect)) goto fw_abort;	/* 这里做了调整 否则无法写入
		                                                 fs->dsect这个是扇区值，目前函数需要的是扇区值
		                                              Send data to the sector */
		//if (disk_writep(p, wcnt)) goto fw_abort;	/*这里是原函数 Send data to the sector */
		fs->fptr += wcnt; p += wcnt;				/* 更新指针和计数器 Update pointers and counters */
		btw -= wcnt; *bw += wcnt;
		if (((WORD)fs->fptr % 512) == 0) {
		  if (disk_writep(0, 0)) goto fw_abort;	/* 完成当前扇区写入操作 Finalize the currtent secter write operation */
			fs->flag &= ~FA__WIP;
		}
	}
